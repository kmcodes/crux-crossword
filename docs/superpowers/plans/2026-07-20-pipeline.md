...
"
```
Measured on the real corpus during plan validation: roughly **half of grids fill
within a 40s budget**, successful ones often in well under a second (0.3s and
32.5s on two sampled grids). Failures are expected and are absorbed by moving to
the next of the ~2,900 patterns — Task 8 loops over patterns rather than
insisting on any one.

If *no* grid fills, the levers in order are: raise `time_budget_s`, lower
`min_score` from 50 to 40 in `CandidatePool.load` (more candidates, slightly
weaker fill), or widen the `head` slice in `_solve`. Do not remove the
clue-coverage constraint.
