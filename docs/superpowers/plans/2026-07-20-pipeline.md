...
- Produces:
  ...
  - `fill_grid(pattern: str, size: int, pool: CandidatePool, seed: int = 0, max_restarts: int = 50, time_budget_s: float = 60.0) -> FillResult | None`
...
restarts for 15s-class budgets. The `max_restarts=50` default is a legacy
signature and is actively harmful at short budgets.
