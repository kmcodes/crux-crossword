...
- Corpus files are large (93MB zip, 273MB extracted). Store under `data/raw/`, git-ignored.
...
- Produces: `SOURCES: dict[str, Source]` where `Source` is a `NamedTuple(url: str, filename: str, sha_optional: str | None)`; `download_all(dest: Path, skip_existing: bool = True) -> list[Path]`; `extract_zip(zip_path: Path, dest: Path) -> Path`.
...
- [ ] **Step 6: Fetch the real corpus (one-time, ~93MB download)**
...
from crossworder.grid import crossings, derive_slots, slot_length_profile
