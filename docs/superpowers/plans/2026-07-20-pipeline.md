...
class ClueBank:
    ...
    def pick(self, answer: str, rng: random.Random) -> tuple[str, bool] | None:
        ...
        # Word-boundary, not substring: a plain `answer not in c.upper()`
        # discards good clues whose answer merely sits inside a longer word.
        # Measured on the real corpus, it rejected "Generation or more" for
        # ERA and "Switch positions" for ONS, and wiped out the entire clue
        # pool for ATI and ALLE — with no genuine giveaway among them.
        gives_away = re.compile(rf"\b{re.escape(answer)}\b").search
